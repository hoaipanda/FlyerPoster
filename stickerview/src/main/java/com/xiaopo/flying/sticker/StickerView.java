...
//                canvas.scale(sticker.getScale(), sticker.getScale());
    ...
    public void setScale(float scale) {
        ...
        moveMatrix.postScale(handlingSticker.getScale() * handlingSticker.getScaleDefault(), handlingSticker.getScale() * handlingSticker.getScaleDefault(),
            ...
            case ActionMode.ZOOM_WITH_TWO_FINGER:
                if (handlingSticker != null) {
                    ...
                    moveMatrix.postScale(newDistance / oldDistance, newDistance / oldDistance,
                            midPoint.x, midPoint.y);
                    handlingSticker.setScaleDefault(newDistance / oldDistance);
    ...
    @NonNull
    public StickerView addSticker(@NonNull Sticker sticker, int x, int y) {
        return add(sticker, x, y);
    }

    @NonNull
    public StickerView addSticker(@NonNull Sticker sticker) {
        return addStickerNone(sticker);
    }

    public StickerView addStickerNone(@NonNull final Sticker sticker) {
        if (ViewCompat.isLaidOut(this)) {
            addStickerImmediatelyNone(sticker);
        } else {
            post(new Runnable() {
                @Override
                public void run() {
    ...
    public StickerView add(@NonNull final Sticker sticker,
    ...
    protected void addStickerImmediatelyNone(@NonNull Sticker sticker) {
        setStickerPositionNone(sticker);
        ...
        sticker.getMatrix()
                .postScale(scaleFactor / 2, scaleFactor / 2, 0, 0);
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, int x, int y) {
    ...
    protected void setStickerPositionNone(@NonNull Sticker sticker) {
        int position = Sticker.Position.CENTER;
        float width = getWidth();
        float height = getHeight();
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, int x, int y) {
        float width = getWidth();
        float height = getHeight();
        float offsetX = width - sticker.getWidth();
        float offsetY = height - sticker.getHeight();
